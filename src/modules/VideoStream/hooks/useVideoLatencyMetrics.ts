...
import { calculateLatencyMs, toDisplayEpochMs } from '../lib/utils'

export const useVideoLatencyMetrics = () => {
  const [latencyMetrics, setLatencyMetrics] = useState<
    Partial<Record<string, VideoLatencyMetric>>
  >({})
  const videosRef = useRef<Partial<Record<string, HTMLVideoElement>>>({})
  const pendingSamplesRef = useRef<
    Partial<Record<string, VideoLatencySampleMessage>>
  >({})
  const callbackHandlesRef = useRef<
  ...
  const scheduleFrameCallback = useCallback((cameraId: string) => {
    ...
    const requestVideoFrameCallback = Reflect.get(
      video,
