...
const findImageStart = (
  bytes: Uint8Array,
): { offset: number; imageType: string } | null => {
...
const getHexPreview = (bytes: Uint8Array, maxLength: number = 32): string => {
  return Array.from(bytes.slice(0, maxLength), (byte) =>
...
const getTextPreview = (bytes: Uint8Array, maxLength: number = 160): string => {
  return textDecoder
    .decode(bytes.slice(0, maxLength))
    .replace(/[^\x20-\x7e]/g, '.')
}
...
const getPayloadDiagnostics = (
  ...
  const payloadBytes = payloadOffset
    ? bytes.subarray(4 + payloadOffset)
    : bytes.subarray(4)
...
const tryExtractFramePayload = (
  ...
  const imageBytes = includeImageBytes
    ? buffer.slice(4 + metaLength)
    : new ArrayBuffer(0)
...
const tryExtractFramePayloadBySignature = (
  ...
  return {
    meta,
    imageBytes: includeImageBytes
      ? buffer.slice(imageStart.offset)
      : new ArrayBuffer(0),
...
function formatBufferData(
  ...
  const view = new DataView(buffer)
  const metadataLengthCandidates = [
...
export { formatBufferData, drawFrame, createSocket }
